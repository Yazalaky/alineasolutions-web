import React, { useState, useEffect } from 'react';
import { ArrowUp } from 'lucide-react';

const WhatsAppIcon = () => (
  <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" className="w-7 h-7">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
  </svg>
);

export const FloatingButtons: React.FC = () => {
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [isOverDark, setIsOverDark] = useState(false);

  useEffect(() => {
    const checkScrollAndContrast = () => {
      // 1. Check Scroll Position
      if (window.scrollY > 300) {
        setShowScrollTop(true);
      } else {
        setShowScrollTop(false);
      }

      // 2. Check Contrast (Adaptive Color)
      // Approximate the position of the scroll button on the screen
      // Bottom offset: 24px (padding) + 60px (whatsapp) + 16px (gap) + ~25px (half button) = ~125px from bottom
      const x = window.innerWidth - 60; // Right side approx
      const y = window.innerHeight - 125; 

      // Get elements at that point
      const elements = document.elementsFromPoint(x, y);
      
      // Check if any element in the stack has data-theme="dark"
      const isDark = elements.some(el => el.closest('[data-theme="dark"]'));
      setIsOverDark(isDark);
    };

    window.addEventListener('scroll', checkScrollAndContrast);
    // Initial check
    checkScrollAndContrast();

    return () => window.removeEventListener('scroll', checkScrollAndContrast);
  }, []);

  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-4 items-end pointer-events-none">
      {/* Scroll to Top Button */}
      {/* pointer-events-auto is needed because parent has pointer-events-none to avoid blocking click on elements below */}
      <button
        onClick={scrollToTop}
        className={`pointer-events-auto p-3 rounded-full shadow-lg transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl focus:outline-none flex items-center justify-center ${
          showScrollTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'
        } ${
          isOverDark 
            ? 'bg-white text-brand-900 border border-gray-200' 
            : 'bg-brand-900 text-white border border-brand-800'
        }`}
        aria-label="Volver arriba"
        title="Volver arriba"
      >
        <ArrowUp className="w-6 h-6" />
      </button>

      {/* WhatsApp Button */}
      <a
        href="https://wa.me/573173638903"
        target="_blank"
        rel="noopener noreferrer"
        className="pointer-events-auto p-4 bg-[#25D366] text-white rounded-full shadow-lg hover:bg-[#20bd5a] transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl flex items-center justify-center group"
        aria-label="Chat de WhatsApp"
        title="Chat con un asesor"
      >
        <WhatsAppIcon />
        <span className="absolute right-full mr-3 bg-white text-gray-800 px-3 py-1 rounded-lg text-sm font-medium shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap hidden md:block pointer-events-none">
          ¡Hablemos!
        </span>
      </a>
    </div>
  );
};